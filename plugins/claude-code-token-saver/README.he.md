...
| 🔄 /cc-continue | מחליף את /compact — אפס קריאות LLM, אפס עלות, אפס אובדן מידע, ומשחזר גם סשנים של **Codex** | שחזור context בחינם בשני הכלים |
...
|                     | `/cc-continue` לבד               | `/cc-compact` + `/cc-continue` (הזוג)             |
...
| מסמך מסירה של `/cc-compact` | משותף לכל פרויקט — כתוב אותו בכלי אחד, טען אותו בשני |

```
/cc-continue codex                    רק סשני Codex
/cc-continue codex : rust migration   התורות התואמות לנושא, משוחזרים במלואם
```
