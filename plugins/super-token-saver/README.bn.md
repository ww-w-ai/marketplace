...
Token Guardian মনে রাখে শেষ রিপ্লাই কখন এসেছিল। যদি ৩,৫৯০ সেকেন্ডের বেশি কেটে যায় (TTL মাইনাস ১০-সেকেন্ড বাফার), এটি হস্তক্ষেপ করে। ডিফল্টে এটি **সতর্ক করে**: prompt চলে যায়, এবং Claude তার রিপ্লাই শুরু করে এক লাইন দিয়ে — ক্যাশের মেয়াদ শেষ হয়ে গিয়েছিল, এই টার্ন পুরো রি-সেন্ড হিসেবে বিল হয়েছে, এবং এক ঘণ্টা বা তার বেশি বিরতির পরে সস্তা পথ হল `/clear` → `/s-continue`।

**কেন warn ডিফল্ট।** আগের সংস্করণগুলি prompt ব্লক করত এবং নিচের সতর্কতা দেখাত। টার্মিনালে সেটি কাজ করে। Remote Control-এ করে না: একটি hook-এর block বার্তা স্থানীয়ভাবে system message হিসেবে রেন্ডার হয়, যা remote client কখনো পায় না — তাই prompt কোনো ব্যাখ্যা ছাড়াই শুধু অদৃশ্য হয়ে যেত। Claude-এর রিপ্লাই *ঠিকই* ফরওয়ার্ড হয়, তাই সতর্কতা এখন তার সাথেই যায়। যারা তাদের সেশন remote থেকে চালান, তাদের জন্য আমরা ডিফল্ট বদলেছি।

আপনি যদি মূলত লোকাল টার্মিনালে কাজ করেন এবং hard stop ফিরে চান:

```
export CC_TOKEN_SAVER_CACHE_GUARD=block
```

block mode-এ prompt নিচের বার্তা সহ একবার প্রত্যাখ্যাত হয়। আবার পাঠান, এটি চলে যাবে। `off` চেকটি সম্পূর্ণ নিষ্ক্রিয় করে।
