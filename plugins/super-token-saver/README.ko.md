...
Token Guardian은 마지막 응답을 받은 시각을 기억합니다. 3,590초(만료 시간에서 10초 여유를 뺀 값)가 지났으면 나섭니다. 기본 동작은 **warn**입니다. 프롬프트는 그대로 통과시키고, Claude가 답변 첫 줄에 "캐시가 만료돼 이 턴은 전체 재전송으로 과금됐다, 한 시간 넘게 쉬었다 올 땐 `/clear` → `/s-continue`가 싸다"고 한 줄 알립니다.

**왜 warn이 기본인가.** 예전 버전은 프롬프트를 막고 아래 경고를 띄웠습니다. 터미널에서는 잘 됩니다. 그런데 원격 제어(Remote Control)에서는 안 됩니다. 훅의 차단 메시지는 로컬 시스템 메시지로만 그려지고 원격 화면에는 전달되지 않아서, 보낸 프롬프트가 아무 설명 없이 사라진 것처럼 보였습니다. Claude의 답변은 원격에도 전달되므로 경고를 거기에 실었습니다. 세션을 원격으로 자주 돌리는 분들 때문에 기본값을 바꿨습니다.

주로 로컬 터미널에서 일하고 예전처럼 확실히 막고 싶다면:

```
export CC_TOKEN_SAVER_CACHE_GUARD=block
```

block 모드에서는 아래 메시지와 함께 프롬프트를 한 번 거부합니다. 같은 프롬프트를 다시 보내면 통과합니다. `off`로 두면 검사를 아예 안 합니다.
