...
> תוצאה מדודה: **הפחתת עלויות של 45%** על עומס עבודה אמיתי של $326/יום → $180/יום. האצלת SubTask אוטומטית, שחזור context ללא עלות, לוח אנליטיקה מלא, ושומר מפני פקיעת cache — בהתקנה אחת, ללא הגדרה.
...
| 🛡️ Token Guardian | מזהיר אותך ברגע שפקיעת cache שולחת מחדש את ה-context שלך, או חוסם את זה במצב `block` | לא עוד הפתעות שקטות של $9 |
...
לגזירת 2,200 טוקנים נסתרים מהוראות git המובנות של CC ([פרטים](#%EF%B8%8F-feature-4-setup-git-lite--trim-ccs-built-in-git-instructions)):
...
## 🔄 תכונה 2: /s-continue — שחזור Context
...
## ✂️ תכונה 4: /setup-git-lite — קצץ את הוראות git המובנות של CC
...
## 🛡️ תכונה 5: Token Guardian

**מודיע לך ברגע שפקיעת cache עולה לך כסף. יכול לחסום את השליחה החוזרת של $9 אם תבקש ממנו.**
...
Token Guardian זוכר מתי התקבלה התשובה האחרונה. אם עברו יותר מ-3,590 שניות (TTL פחות חיץ של 10 שניות), הוא יכול להתערב. **הוא כבוי כברירת מחדל, בגלל Remote Control.** הודעת החסימה של hook מוצגת מקומית כהודעת מערכת שהלקוח המרוחק לעולם לא מקבל, כך שמשתמש מרוחק ראה את ה-prompt נעלם בלי שום הסבר. במקום לשלוח הגנה שמתנהגת אחרת בהתאם למקום שבו אתה נמצא, כיבינו אותה. כש-Remote Control יתחיל להעביר הודעות hook, ברירת המחדל תחזור לפעול. עד אז, הפעל אותה בעצמך באחד משני המצבים.

```
export CC_TOKEN_SAVER_CACHE_GUARD=warn    # Claude מזכיר את הפקיעה בשורה הראשונה שלו
export CC_TOKEN_SAVER_CACHE_GUARD=block   # ה-prompt נדחה פעם אחת עם ההודעה שלמטה
```

במצב `warn` ה-prompt עובר, ו-Claude פותח את תשובתו בשורה אחת שאומרת שה-cache פג, שהתור הזה חויב כשליחה חוזרת מלאה של ה-context, ושאחרי הפסקה של שעה או יותר `/clear` → `/s-continue` היא הדרך הזולה יותר לחזור. זו כן מגיעה ללקוח מרוחק, כי תשובת Claude מועברת גם כשהודעות hook לא.

במצב `block` ה-prompt נדחה פעם אחת עם ההודעה שלמטה. שלח אותו שוב והוא יעבור. השתמש בו בטרמינל מקומי כשאתה רוצה את העצירה הקשיחה.
...
הודעת החסימה מוצגת ב-23 שפות, לפי ה-locale של מערכת ההפעלה שלך, ומופעלת פעם אחת לכל תקופת חוסר פעילות.

**סוכני רקע (background agents) לעולם לא נחסמים.** רק prompts שאדם הקליד בעצמו מקבלים את הבדיקה. דוחות השלמה מסוכני רקע ומשימות, שכיום מגיעים לעתים קרובות יותר משעה אחרי שהופעלו, עוברים ישירות. התוצאה של סוכן שרץ זמן ארוך לעולם לא מתעכבת או אובדת.

**תוצאה:** במצב warn אתה תמיד יודע מתי קרתה שליחה חוזרת של $9, ולמה. במצב block זה לא קורה: כל פקיעת cache שנלכדת = $9 חסכון, וב-catch אחד ביום זה $270/חודש של בזבוז טהור שנמנע.

> **אם אתה משתמש ב-API לפי שימוש, זה פוגע יותר קשה.** מנויי Max Plan מאבדים $9 בתוך חיץ של $200. אתה מאבד $9 כסף אמיתי — בשקט, שוב ושוב, בכל פעם שאתה מתרחק. Token Guardian במצב block עוצר את זה בכל פעם.
...
| צהריים/ישיבה | משך 2 שעות                                 | —                           | —                                  |
| חזרה        | ⚡ Token Guardian (מצב block) → /clear + /s-continue | —                           | ＄0 (אין קריאות LLM)               |
...
[חוסר פעילות 1+ שעה]
    │
    ├─ Token Guardian → מזהה פקיעת cache, מזהיר (או חוסם במצב block)
    │
[הפעלה מחדש של סשן]
    │
...
| אזהרת פקיעת cache | בחוסר פעילות > 59 דקות (פעם אחת) | ~200 | מסמן את השליחה החוזרת היקרה, מציג את הדרך הזולה יותר |
...
- **טיימר ה-cache מאופס מקריאת API האחרונה, לא מ-prompt האחרון שלך.** המשך לעבוד וה-cache לעולם לא יפוג. הסכנה היא ההתרחקות. Token Guardian מודיע לך כשזה קרה, ובמצב `block` עוצר את ה-prompt פעם אחת כדי שתוכל לבחור: אפס את ה-context, או המשך כפי שהוא.
