...
Token Guardian זוכר מתי התקבלה התשובה האחרונה. אם עברו יותר מ-3,590 שניות (TTL פחות חיץ של 10 שניות), הוא נכנס לפעולה. כברירת מחדל הוא **מזהיר**: ה-prompt עובר, ו-Claude פותח את התשובה שלו בשורה אחת שאומרת שה-cache פג, שהתור הזה חויב כשליחה חוזרת מלאה, ושאחרי הפסקה של שעה או יותר הדרך הזולה יותר היא `/clear` → `/s-continue`.

**למה warn היא ברירת המחדל.** גרסאות קודמות חסמו את ה-prompt והציגו את האזהרה שלמטה. זה עובד בטרמינל. תחת Remote Control זה לא עובד: הודעת החסימה של hook מוצגת מקומית כהודעת מערכת שהלקוח המרוחק לעולם לא מקבל, כך שה-prompt פשוט נעלם בלי הסבר. התשובה של Claude *כן* מועברת, אז האזהרה רוכבת עכשיו עליה במקום. שינינו את ברירת המחדל עבור אנשים שמפעילים את הסשנים שלהם מרחוק.

אם אתה עובד בעיקר בטרמינל מקומי ורוצה את העצירה הקשיחה בחזרה:

```
export CC_TOKEN_SAVER_CACHE_GUARD=block
```

במצב block ה-prompt נדחה פעם אחת עם ההודעה שלמטה. שלח אותו שוב והוא יעבור. `off` מבטל את הבדיקה לחלוטין.
