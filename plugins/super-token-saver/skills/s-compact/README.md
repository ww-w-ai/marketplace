...
Write a **session handoff** for the next session — the write-side pair of `/s-continue`.

## Usage

```
/s-compact          # generate the handoff, save it, print it, copy to clipboard
/s-compact help     # usage
```

Pairs with `/s-continue`: **end a session with `/s-compact`, start the next with `/s-continue`.**

## Why it exists

`/s-continue` restores the transcript — the messages you and Claude exchanged. But a working
...
`/s-compact` distills exactly those into a handoff, so the next session doesn't re-derive them.
...
3. Saves it to `~/.claude/super-token-saver-data/<project>/handoff.md` — the exact path
   `/s-continue` auto-loads.
...
| | |
|---|---|
| End of session | `/s-compact` — write the handoff |
| Start of next session | `/s-continue` — restore transcript **+ auto-load the handoff** |
