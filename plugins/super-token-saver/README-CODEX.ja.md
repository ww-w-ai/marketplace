...
- **ホスト別にフックを分離する。** Codex には Codex 専用のセッション指針と圧縮復元だけを注入し、Claude Code のプロンプトキャッシュ、ステータスライン、Git コンテキスト用フックは実行しない。
...
/s-continue last --level 1              浅く復元する — 流れだけ、分量は省く
/s-compact            次の人のための引き継ぎを書く
```

`~/.codex` 以外の場所に Codex の状態を置いている場合は ## どこまで復元するか

`--level 1|2|3`（既定は 3）で、各セッションをどこまで深く読むかを決める。

| レベル | ターン | 両端の応答 | 中間の応答 | 170ターンのセッションでの実測 |
|---|---|---|---|---|
| 1 | 直近 30、150 + 100 文字に短縮 | 先頭 6 + 末尾 6、各 100 文字 | 50 文字 | 6.1K トークン |
| 2 | 直近 30、保存されたまま | 先頭 12 + 末尾 12、そのまま | 50 文字 | 9.3K トークン |
| 3 | すべて | 先頭 24 + 末尾 24、そのまま | 50 文字 | 44.3K トークン |

**捨てられるターンも応答もない。**長いターンの中間が短くなるだけで、消えはしない。各ターンには元の
rollout を指す `L{n}` と応答行の範囲が残るため、短縮された箇所が必要になった時点で全文を読み直せる。

自律実行のあとは 1 つのユーザーターンに数百の応答が付く。そこで持ちこたえるのはレベル 1 だ。
