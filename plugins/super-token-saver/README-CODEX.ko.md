...
호스트가 스스로 압축하면, 플러그인의 압축 후 훅이 사용자가 무엇을 입력하기 전에 잘려 나간 턴을
되돌려 놓는다. 사용자의 턴, 모델의 답변, 팀메이트 메시지와 서브에이전트 알림을 원문 그대로 넣고
도구 호출만 뺀다. 세션마다 원장을 쌓아 두므로, 압축이 여러 번 일어나거나 같은 프로젝트의 세션이
여러 개 이어져도 그 전 턴들이 접힌 형태로 토큰 예산 안에서 따라온다. 모든 줄에 원본 rollout 을
가리키는 `L{n}` 표시가 남는다.

`/s-continue` 는 고른 세션의 모든 턴을 복원한다. 긴 턴은 앞뒤 24개 답변을 저장된 그대로 두고
가운데는 50자로 줄이며, 버리는 것은 없다.
