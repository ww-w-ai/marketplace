...
- **호스트별 훅을 분리한다.** Codex에는 Codex 전용 세션 지침과 압축 복원만 주입하며, Claude Code의 프롬프트 캐시·상태줄·Git 컨텍스트 훅은 실행하지 않는다.
...
/s-continue last --level 1              얕게 복원한다 — 흐름만, 분량은 빼고
/s-compact            다음 사람을 위한 인계 기록을 남긴다
```

`~/.codex` 가 아닌 다른 곳에 Codex 상태를 두었다면 ## 얼마나 복원할지

`--level 1|2|3` (기본 3) 으로 세션을 얼마나 깊이 읽을지 정한다.

| 레벨 | 턴 | 양 끝 응답 | 가운데 응답 | 170턴 세션 실측 |
|---|---|---|---|---|
| 1 | 마지막 30개, 150 + 100자 | 앞 6 + 뒤 6, 각 100자 | 50자 | 6.1K 토큰 |
| 2 | 마지막 30개, 저장된 그대로 | 앞 12 + 뒤 12, 그대로 | 50자 | 9.3K 토큰 |
| 3 | 전부 | 앞 24 + 뒤 24, 그대로 | 50자 | 44.3K 토큰 |

**버려지는 턴도 응답도 없다.** 긴 턴의 가운데가 짧아질 뿐 사라지지 않는다. 턴마다 원본 rollout 을
가리키는 `L{n}` 표시와 응답 줄 범위가 남아 있어, 짧아진 것이 중요해지는 순간 전문을 다시 읽으면 된다.

자율 실행 뒤에는 사용자 턴 하나에 응답이 수백 개 달리는데, 그때 버티는 것은 레벨 1이다.
